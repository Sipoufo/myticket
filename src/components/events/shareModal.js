...
const ShareModal = ({ title, handleClose }) => {
  ...
  const twitterShare = () => {
    window.open(`https://twitter.com/intent/tweet?url=${url}&text=${encodeURI(description)}`, '_blank');
    handleClose();
  };
  const igShare = () => {
      ...
      <div className="fixed inset-0 z-10 w-screen overflow-y-auto">
        <div className="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
            ...
            <div className="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
              <div className="sm:flex sm:items-start">
                ...
                <div className="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                  <h3 className="text-base font-semibold leading-6 text-gray-900" id="modal-title">Event Sharing</h3>
                  <div className="mt-2">
                    <p className="text-sm text-gray-500">Share <strong>{title}</strong> event's with others using the link or options below.</p>
                    <div class="my-4">
          ...
          <div class="border-2 border-gray-200 flex justify-between items-center mt-4 py-2">
            ...
            <input class="w-full outline-none bg-transparent" type="text" placeholder="link" ref={urlRef} value={url}/>

            <button class="bg-indigo-500 text-white rounded text-sm py-2 px-5 mr-2 hover:bg-indigo-600" onClick={handleCopyUrl}>
                Copy
            </button>
          </div>
        </div>
            ...
            <div className="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
              <button onClick={() => handleClose()} type="button" className="inline-flex w-full justify-center rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50 sm:w-auto">Close</button>
            </div>
          </div>
        </div>
      </div>
